...
export default function Subjects() {
  ...
  const totalMarksList = marks.filter((mark) => mark.student_id === user.id);
  ...
  const avg = Math.floor(sum / subcount);
  return (
    <ScrollView>
      <View>
        <Banner />
          ...
          <View>
            {subjectList.map((subject, index) => (
              <View key={index} style={styles.tableData}>
                ...
                <Text>{totalMarksList.find((mark) => mark.subject_id === subject.id)?.marks}</Text>
              </View>
            ))}
          </View>
        </View>
